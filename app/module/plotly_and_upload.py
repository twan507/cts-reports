    ...
    for attempt in range(max_retries):
        try:
            ...
            authorization = _aws_signature_v4(
                "PUT", upload_url, headers, data_bytes, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, REGION, "s3", amz_date
            )
            headers["Authorization"] = authorization

            session = requests.Session()
            adapter = requests.adapters.HTTPAdapter(
                max_retries=requests.packages.urllib3.util.retry.Retry(total=3, backoff_factor=0.3, status_forcelist=[500, 502, 503, 504])
            )
            session.mount("https://", adapter)

            response = session.put(upload_url, data=data_bytes, headers=headers, timeout=(30, 60), verify=True)
...
def create_ticker_chart(df, height, width, path, png_name="chart.png"):
    ...
    fig = px.line(df_chart, x="date_label", y="close", markers=True)
